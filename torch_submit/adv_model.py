...
class StyleTrans(object):
    ...
    def _init_seg(self):
        if self.opt.seg_net == 'fcn':
            model = FCN8s(self.opt.n_classes, pretrained=False)
            file_name = os.path.join(self.opt.resume, self.opt.fcn_name)
        elif self.opt.seg_net == 'deeplab_ibn':
            deeplab = resnet101_ibn_a_deeplab()
            file_name = os.path.join(self.opt.resume, 'deeplab_ibn.pth')
    ...
    def _init_discrim(self):
        ...
        self.D_pool = PoolingDiscriminator()
    ...
    def compute_discrim_loss(self, x, domain_label):
        ...
        handle = self.seg.module.features3.register_forward_hook(get_output)

        new_x = self.fcn_normalize(x)
        domain_output = self.D(self.seg(new_x))
        ...
        return discrim_loss + pool_discrim_loss
        # return discrim_loss
...
class _AddNorm(object):
    def __init__(self, val_net, de_normalize, normalize):
        self.val_net = val_net
        self.de_normalize = de_normalize
        self.normalize = normalize
        self.upsample = torch.nn.Upsample(
