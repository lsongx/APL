...
def main(args):
    ...
    if args.seg_net == 'fcn':
        net = FCN8s(19, pretrained=False)
        net_static = FCN8s(19, pretrained=False)
    ...
    print('Test with Gen')
    mean_iu = test_miou(new_model, val_loader_direct, upsample, './dataset/info.json')
    # return
    ...
    for epoch in range(args.num_epoch):
        ...
        for batch_index, batch_data in enumerate(tgt_loader):
            ...
            with torch.no_grad():
                ...
                gen_seg_output_logits = net_static(normalize(de_normalize(gen_output)))
            ...
            for tem_label in range(19):
                ...
                large_value = torch.topk(value_vec, int(args.percent*value_vec.shape[0]))[0][0]
            ...
            if iteration % args.checkpoint_freq == 0:
                mean_iu = test_miou(net, val_loader, upsample, './dataset/info.json', print_results=False)
                if mean_iu > highest:
                    torch.save(
    ...
    net.module.load_state_dict(
        torch.load(os.path.join(args.save_path_prefix, 'cityscapes_best_fcn.pth'))
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--resume", type=str, default='/mnt/data-1/data/liangchen.song/trained_models')
