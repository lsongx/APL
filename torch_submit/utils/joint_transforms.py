...
class FreeScale(object):
    def __init__(self, size):
        self.size = tuple(reversed(size))  # input_size: (h, w)
        ...
        if long_size > self.crop_size:
            ...
            for yy in xrange(h_step_num):
                for xx in xrange(w_step_num):
                    ...
                    img_sublist.append(Image.fromarray(
                        img_sub.astype(np.uint8)).convert('RGB'))
                    mask_sublist.append(Image.fromarray(
                        mask_sub.astype(np.uint8)).convert('P'))
            ...
            for yy in xrange(h_step_num):
                for xx in xrange(w_step_num):
                    ...
                    img_sub, mask_sub, sub_h, sub_w = self._pad(
                        img_sub, mask_sub)
                    img_slices.append(Image.fromarray(
                        img_sub.astype(np.uint8)).convert('RGB'))
                    mask_slices.append(Image.fromarray(
                        mask_sub.astype(np.uint8)).convert('P'))
