...
class FCN8s(nn.Module):
    def __init__(self, num_classes, model_path_prefix=None, pretrained=True):
        super(FCN8s, self).__init__()
        vgg = models.vgg16()
        if pretrained:
            vgg.load_state_dict(torch.load(
                # os.path.join(model_path_prefix, 'vgg16-397923af.pth')
                os.path.join(model_path_prefix, 'vgg16_from_caffe.pth')
            ))
