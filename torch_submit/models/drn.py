...
def drn_c_26_path(pretrained=False, model_path_prefix=None, **kwargs):
    model = DRN(BasicBlock, [1, 1, 2, 2, 2, 2, 1, 1], arch='C', **kwargs)
    if pretrained:
        model.load_state_dict(torch.load(
            os.path.join(model_path_prefix, 'drn_c_26-ddedf421.pth')
        ))
    return model
...
class DRNSeg(nn.Module):
    model_list = {
        'drn_c_26': drn_c_26_path,
    }

    def __init__(self, classes, pretrained=True, model_name='drn_c_26',
                 model_path_prefix=None, use_torch_up=False):
